...
import './css/repertuar.css';
import React,{useState,useEffect} from 'react';
import axios from 'axios';
import {Link} from 'react-router-dom';

function Repertuar({navigation, Route}) {
  ...
  const checkCurrentSeanses = () => {
    ...
    const m = 4;
    const n = 2; 
    let currentSeansesTable = new Array(m); 
    for (var i = 0; i < m; i++) {
      currentSeansesTable[i] = new Array(n); 
    }
  ...
  return (
    <div className='cont'>
      ...
      <div className="row">
        ...
        {seansesToDisplay.map(seans =>(
          <div className="column">
            {/* <Link  id="link" to="/reservation"> */}
            <Link style={{textDecoration: 'none',}}  to={
                {     
                  pathname: '/reservation?id_se='+seans.id_se,
                  state: seans.id_se,
                }
            }> 
            <h2>{seans.tytul}</h2>
            {/* <a href={cos.link} target="_blank" rel="noopener noreferrer"> */}
              <img style={{paddingLeft: "15%"}} src={seans.zdjecie}></img>
            {/* </a> */}
            <h3>{seans.godzina.slice(0, 5)}</h3>
